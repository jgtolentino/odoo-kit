...
const client = createClient({ url: 'https://api.supabase.com', fetch: fetch })
const openai = new OpenAI({ apiKey: process.env.OPENAI_API_KEY })

// Function to get database schema
async function getDbSchema(projectRef: string, client: ReturnType<typeof createClient<paths>>) {
...
export async function POST(request: Request) {
  try {
    ...
    const schema = await getDbSchema(projectRef, client)
